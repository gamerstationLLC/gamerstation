// app/api/tools/lol/_earena/route.ts
...
export async function GET(req: Request) {
  try {
    ...
    // Arena queues: 1700 and sometimes 1701. Matchlist only takes ONE queue param,
    // so we fetch both and merge until we have enough.
    const ids1700Url = `https://${cluster}.api.riotgames.com/lol/match/v5/matches/by-puuid/${encodeURIComponent(
      puuid
    )}/ids?start=0&count=${limit}&queue=1700`;

    const ids1701Url = `https://${cluster}.api.riotgames.com/lol/match/v5/matches/by-puuid/${encodeURIComponent(
      puuid
    )}/ids?start=0&count=${limit}&queue=1701`;

    const [a, b] = await Promise.all([
      riotJson<string[]>(ids1700Url).catch(() => [] as string[]),
      riotJson<string[]>(ids1701Url).catch(() => [] as string[]),
    ]);

    const merged = Array.from(new Set([...(a ?? []), ...(b ?? [])])).slice(0, limit);

    const out: MatchRow[] = [];
    for (const matchId of merged) {
